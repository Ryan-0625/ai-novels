...
@file: llm/adapters/minimax.py
@date: 2026-03-12
@author: AI-Novels Team
@version: 1.0
@description: MiniMax API客户端适配器，支持MiniMax-Text-01
"""
...
class MinimaxClient(BaseLLMClient, EmbeddingClient):
    """
    MiniMax LLM客户端适配器
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        Args:
            config: Minimax配置
                ...
                - base_url: API地址 (默认: https://api.minimax.chat)
                - timeout: 请求超时秒数 (默认: 30)
        """
        super().__init__(config)
        try:
            from minimax import MinimaxClient as MinimaxAPIClient
            self._client = MinimaxAPIClient(
                api_key=config.get("api_key"),
                group_id=config.get("base_url")
            )
        except ImportError:
            raise ImportError("Please install minimax: pip install minimax")

        self._model = config.get("model", "MiniMax-Text-01")
